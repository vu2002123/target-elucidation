...
gene_file = INTERIM_DIR / "PCP_only_intersection_list_20percent.txt"
...
outfile = Path.home() / "target-elucidation" / "reports" / "figures" / "depmap_luad.png"
...
order = df_melted.groupby("Gene")["CRISPR Gene Effect"].median().sort_values().index[:15]
...
outfile = (
    ...
    / "reports"
    / "figures"
    / "depmap_CY001_only_nsclc_vs_pdac.png"
)
plt.savefig(outfile, bbox_inches="tight", dpi=600)
plt.close()
...
for code, disease in cohorts.items():
    ...
    outfile = FIG_DIR / f"highlight_analysis_{code.lower()}.png"
    plt.savefig(outfile, bbox_inches="tight", dpi=600)
    plt.close()
